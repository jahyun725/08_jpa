...
class EntityLifeCycleTest {
  ...
  void testManagedSameEntityManager(int menuCode) {
    //given
    // 엔티티 매니저를 1개 얻어옴
    // -> 해당 엔티티 매니저가 관리하는 영속성컨텍스트(persist context)가 별도로 존재
    ...
    //  -> 1차 캐시에 저장됨, 똑같은 menuCode를 통해 조회할 경우 재사용 됨
  ...
  void testManagedOtherEntityManager(int menuCode) {
    //when
    // lifeCycle.findMenuByMenuCode() 호출 시마다
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    // (더티체킹 : @Entity 객체의 변화(원본(DB)과 snapshot의 차이)를 감지)
    ...
    *
    * 현재 영속 상태의 객체가 없으므로 아무 일도 일어나지 않음
    * */
    entityManager.flush(); // DB에 전달만 한 상태(commit,rollback) 가능
    ...
    // 사실 foundMenu가 준영속상태라 commit해도 결과가 바뀌지 않음
  }

  @DisplayName("준영속화 detach 후 다시 영속화 테스트")
  @ParameterizedTest
  @CsvSource({"103, 7000", "12, 1000"})
  void testDetachAndMerge(int menuCode, int menuPrice) {
    ...
    // then
    assertEquals(menuPrice, checkMenu.getMenuPrice());
    entityTransaction.rollback();
  }

  @DisplayName("detach후 merge한 데이터 update 테스트")
  @ParameterizedTest
  @CsvSource({"103, 두바이쫀득쿠키"})
  void testMergeUpdate(int menuCode, String menuName) {
    ...
    transaction.commit(); //update 수행 됨
  ...
  void testMergeSave() {
    ...
    //pk 수정
    // menu entity class의 @GeneratedValue(strategy=GenerationType.IDENTITY) 주석처리
    foundMenu.setMenuCode(999);

    // 1차 캐시와 준영속 객체를 병합하여 영속 객체를 반환
    // -> 더티체킹 -> menu_code(PK)가 999인 행이 없음을 확인함
    // -> INSERT 구문 생성
